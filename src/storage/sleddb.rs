...
use crate::{Storage, StorageIter, Kvpair};
...
impl SledDb {
    ...
    // 在sleddb里， 可以使用scan_prefix，我们用prefix来模拟table。
    fn get_full_key(table: &str, key: &str) -> String {
        format!("{} : {}", table, key)
    }

    // 遍历table的key时候，我们直接把prefix当成table
    fn get_table_prefix(table: &str) -> String {
        format!("{}", table)
    }
}

/// 把Option<Result<T, E>> flip 成 Result<Opition<T>, E>
/// 从这个函数里，可以看到函数式编程的优雅
...
impl Storage for SledDb {
    ...
    fn set(&self, table: &str, key: String, value: crate::Value) -> Result<Option<crate::Value>, crate::KvError> {
        ...
        let result = self.0.insert(name, data)?
            .map(|v| v.as_ref().try_into());
        flip(result)
    }
